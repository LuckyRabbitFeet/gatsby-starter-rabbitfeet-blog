...
类似的状况也会发生在 `<` 符号上，因为 Markdown 允许 [兼容 HTML](https://www.appinn.com/markdown/index.html#html) ，如果你是把 `<` 符号作为 HTML 标签的定界符使用，那 Markdown 也不会对它做任何转换，但是如果你写：
...
的确，需要多费点事（多加空格）来产生 `<br />` ，但是简单地「每个换行都转换为 `<br />`」的方法在 Markdown 中并不适合， Markdown 中 email 式的 [区块引用](https://www.appinn.com/markdown/index.html#blockquote) 和多段落的 [列表](https://www.appinn.com/markdown/index.html#list) 在使用换行来排版的时候，不但更好用，还更方便阅读。
